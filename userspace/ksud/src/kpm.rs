...
pub fn handle_kpm_modules() {
    tokio::spawn(async {
        if let Err(e) = process_kpm_modules().await {
            warn!("kpm modules processing failed, but system continues: {}", e);
        }
    });
}

async fn process_kpm_modules() -> Result<()> {
    let result = timeout(Duration::from_secs(30), async {
        let kpm_enabled = timeout(Duration::from_secs(5), async {
            tokio::task::spawn_blocking(|| kpm::is_kpm_enabled()).await?
        }).await??;

        if !kpm_enabled {
            info!("kpm not enabled, skipping kpm initialization");
            return Ok(());
        }

        info!("Starting kpm initialization...");

        timeout(Duration::from_secs(5), async {
            tokio::task::spawn_blocking(|| kpm::start_kpm_watcher()).await?
        }).await??;

        timeout(Duration::from_secs(20), async {
            tokio::task::spawn_blocking(|| kpm::load_kpm_modules()).await?
        }).await??;

        info!("kpm initialization completed successfully");
        Ok(())
    }).await;
    ...
    match result {
        Ok(Ok(())) => Ok(()),
        Ok(Err(e)) => {
            warn!("kpm initialization failed: {}", e);
            Err(e)
        }
        Err(_) => {
            warn!("kpm initialization timed out");
            Err(anyhow::anyhow!("kpm initialization timeout"))
        }
    }
}
